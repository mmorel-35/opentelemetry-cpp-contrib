...
|*ApacheModuleOtelResourceAttributes*            |                 | OPTIONAL: Add a custom OpenTelemetry resource attribute as a key-value pair (e.g. `ApacheModuleOtelResourceAttributes deployment.environment production`). Can be specified multiple times to add multiple attributes. Per-VirtualHost configuration is supported. |
