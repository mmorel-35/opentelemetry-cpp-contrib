...
|*ApacheModuleOtelResourceAttributes*            |                 | OPTIONAL: Add a custom OpenTelemetry resource attribute as a key-value pair (e.g. `ApacheModuleOtelResourceAttributes deployment.environment production`). Can be specified multiple times to add multiple attributes. Keys and values must not contain `=` or `,`. Note: attributes are sourced from the VirtualHost that processes the first request and apply for the lifetime of the worker process. |
